...
ens.nippo1 <- transform(ens.nippo, pvalue=sapply(pval, formatC, format = "e", digits=3))
ens.nippo1 <- transform(ens.nippo1, padj=sapply(padj, formatC, format = "e", digits=3))
ens.nippo1 <-ens.nippo1  %>% mutate_at(vars("BaseMean", "Log2FC", "StdErr", "WaldStat"), funs(round(., 3)))
...
ens.dss1 <- transform(ens.dss, pvalue=sapply(pval, formatC, format = "e", digits=3))
ens.dss1 <- transform(ens.dss1, padj=sapply(padj, formatC, format = "e", digits=3))
ens.dss1 <-ens.dss1  %>% mutate_at(vars("BaseMean", "Log2FC", "StdErr", "WaldStat"), funs(round(., 3)))
...
ens.th171 <- transform(ens.th17, pvalue=sapply(pval, formatC, format = "e", digits=3))
ens.th171 <- transform(ens.th171, padj=sapply(padj, formatC, format = "e", digits=3))
ens.th171 <-ens.th171  %>% mutate_at(vars("BaseMean", "Log2FC", "StdErr", "WaldStat"), funs(round(., 3)))
...
multi_gp_ens_nippo = gost(list("up-regulated" = up.genes.ordered$GeneName,
...
multi_gp_ens_dss = gost(list("up-regulated" = up.genes.ordered$GeneName,
                     "down-regulated" = dn.genes.ordered$GeneName),organism = "mmusculus", ordered_query = TRUE)
...
multi_gp_ens_th17 = gost(list("up-regulated" = up.genes.ordered$GeneName,
                     "down-regulated" = dn.genes.ordered$GeneName),organism = "mmusculus", ordered_query = TRUE)
...
```{r GSEA clusterProfiler DSS, message = FALSE, echo=FALSE, warning=FALSE, results='hide'}
...
```{r GSEA clusterProfiler Nippo, message = FALSE, echo=FALSE, warning=FALSE, results='hide'}

###### ENS #################################


ens.nippo2 <- ens.nippo[abs(ens.nippo$Log2FC) > 1.5,]
...
```{r GSEA clusterProfiler Th17, message = FALSE, echo=FALSE, warning=FALSE, results='hide'}

### Th17 ###



ens.th172 <- ens.th17[abs(ens.th17$Log2FC) > 1.5,]
